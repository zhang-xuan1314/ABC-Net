...
# imgs, atom_targets,atom_types,atom_charges,bond_targets,bond_types,bond_rhos,bond_omega_types = train_dataset[0]
# plt.subplot(131)
# plt.imshow(imgs[0])
# plt.subplot(132)
# plt.imshow(atom_targets[0]) plt.subplot(133)
...
#model.load_state_dict(torch.load('weights0.2/unet_model_weights20.pkl'))
...
for epoch in range(epoch_nums):
    ...
    for i,(imgs, atom_targets,atom_types,atom_charges,atom_hs,
        ...
        atom_targets_pred, atom_types_pred, atom_charges_pred,atom_hs_pred,bond_targets_pred, bond_types_pred,bond_rhos_pred, bond_omega_types_pred = model(imgs)
        ...
        atom_types_loss = torch.sum(-atom_type_weights*atom_types*(1-atom_types_pred)**2*torch.log(atom_types_pred))/torch.sum(atom_types)
        ...
        atom_hs_loss = torch.sum(-atom_hs*(1-atom_hs_pred)**2*torch.log(atom_hs_pred))/(torch.sum(atom_hs)+0.1)
        ...
        bond_rhos_loss = torch.sum(torch.abs(bond_rhos_pred-bond_rhos)*torch.sum(bond_types,dim=1))/torch.sum(bond_types)


        bond_omega_types_loss = -torch.sum(torch.sum(bond_omega_types,dim=1,keepdim=True)*((bond_omega_types==1)* ((1-bond_omega_types_pred)**2) * torch.log(bond_omega_types_pred)+
                                               (1-bond_omega_types)**4 * (bond_omega_types_pred**2) * torch.log(1-bond_omega_types_pred)))/torch.sum(bond_omega_types)
        ...
        train_atom_targets_precision.update(((atom_targets_pred * atom_targets).sum() / atom_targets_pred.sum()).cpu().detach().numpy(),atom_targets_pred.sum().cpu().detach().numpy())
        train_atom_targets_precision3.update(((atom_targets_pred * torch.nn.functional.max_pool2d(atom_targets, kernel_size=3, padding=1,
                stride=1)).sum() / atom_targets_pred.sum()).cpu().detach().numpy(),atom_targets_pred.sum().cpu().detach().numpy())

        train_atom_targets_recall.update(((atom_targets * atom_targets_pred).sum() / atom_targets.sum()).cpu().detach().numpy(),
            atom_targets.sum().cpu().detach().numpy())
        train_atom_targets_recall3.update(((atom_targets * torch.nn.functional.max_pool2d(atom_targets_pred, kernel_size=3, padding=1,
                        stride=1)).sum() / atom_targets.sum()).cpu().detach().numpy(),atom_targets.sum().cpu().detach().numpy())

        train_atom_types_acc.update((torch.sum(torch.sum(atom_types, dim=1) * ((atom_types.argmax(1) ==
                    atom_types_pred.argmax(1)).float())) / torch.sum(atom_types)).cpu().detach().numpy(),torch.sum(atom_types).cpu().detach().numpy())

        train_atom_charges_acc.update((torch.sum(torch.sum(atom_charges, dim=1) * ((atom_charges.argmax(1) ==
                atom_charges_pred.argmax(1)).float())) / torch.sum(atom_charges)).cpu().detach().numpy(),torch.sum(atom_charges).cpu().detach().numpy())

        train_atom_hs_acc.update((torch.sum(torch.sum(atom_hs, dim=1) * ((atom_hs.argmax(1) ==
                atom_hs_pred.argmax(1)).float())) / (0.01+torch.sum(atom_hs))).cpu().detach().numpy(),(0.01+torch.sum(atom_hs)).cpu().detach().numpy())

        bond_targets = (bond_targets==1).float()

        train_bond_targets_precision.update(((bond_targets_pred * bond_targets).sum() / bond_targets_pred.sum()).cpu().detach().numpy(),bond_targets_pred.sum().cpu().detach().numpy())
        train_bond_targets_precision3.update(((bond_targets_pred * torch.nn.functional.max_pool2d(bond_targets, kernel_size=3, padding=1,
                                        stride=1)).sum() / bond_targets_pred.sum()).cpu().detach().numpy(),bond_targets_pred.sum().cpu().detach().numpy())

        train_bond_targets_recall.update(((bond_targets * bond_targets_pred).sum() / bond_targets.sum()).cpu().detach().numpy(),bond_targets.sum().cpu().detach().numpy())
        train_bond_targets_recall3.update(((bond_targets * torch.nn.functional.max_pool2d(bond_targets_pred, kernel_size=3, padding=1,
                                stride=1)).sum() / bond_targets.sum()).cpu().detach().numpy(),bond_targets.sum().cpu().detach().numpy())

        train_bond_types_acc.update((torch.sum(torch.sum(bond_types, dim=1) * ((bond_types.argmax(1) ==bond_types_pred.argmax(1)).float())) / torch.sum(
        ...
        temp = torch.cat([bond_omega_types_pred[:,59:],bond_omega_types_pred,bond_omega_types_pred[:,:1]],dim=1).permute(0,2,3,1).reshape(-1,128*128,62)

        temp = ((torch.nn.functional.max_pool1d(temp,stride=1,kernel_size=3,padding=0).reshape(-1,128,128,60).permute(0,3,1,2)==bond_omega_types_pred)* \
        ...
        train_bond_omega_recall3.update((((bond_omega_types)*temp2).sum()/(bond_omega_types).sum()).cpu().detach().numpy(),
                                          bond_omega_types.sum().cpu().detach().numpy())

        train_bond_omega_recall.update((((bond_omega_types) * temp).sum()/(bond_omega_types).sum()).cpu().detach().numpy(),
                                          bond_omega_types.sum().cpu().detach().numpy())
        ...
        if i%100 == 0:
            ...
            with torch.no_grad():
                ...
                for j, (imgs, atom_targets, atom_types, atom_charges,atom_hs,
                    ...
                    atom_targets = (atom_targets==1).float()
                    test_atom_targets_precision.update(((atom_targets_pred * atom_targets).sum() / atom_targets_pred.sum()).cpu().detach().numpy(),atom_targets_pred.sum().cpu().detach().numpy())
                    test_atom_targets_precision3.update(((atom_targets_pred * torch.nn.functional.max_pool2d(atom_targets, kernel_size=3, padding=1,
                        stride=1)).sum() / atom_targets_pred.sum()).cpu().detach().numpy(),atom_targets_pred.sum().cpu().detach().numpy())

                    test_atom_targets_recall.update(((atom_targets * atom_targets_pred).sum() / atom_targets.sum()).cpu().detach().numpy(),
                        atom_targets.sum().cpu().detach().numpy())
                    test_atom_targets_recall3.update(((atom_targets * torch.nn.functional.max_pool2d(atom_targets_pred, kernel_size=3, padding=1,
                        stride=1)).sum() / atom_targets.sum()).cpu().detach().numpy(),atom_targets.sum().cpu().detach().numpy())
                    ...
                    bond_omega_types = (bond_omega_types == 1)
                    test_bond_omega_precision.update((((bond_omega_types)*temp).sum()/(temp).sum()).cpu().detach().numpy(),
                                          temp.sum().cpu().detach().numpy())
                    ...
                    test_bond_omega_recall3.update((((bond_omega_types)*temp2).sum()/(bond_omega_types).sum()).cpu().detach().numpy(),
                                          bond_omega_types.sum().cpu().detach().numpy())

                    test_bond_omega_recall.update((((bond_omega_types) * temp).sum()/(bond_omega_types).sum()).cpu().detach().numpy(),
                                          bond_omega_types.sum().cpu().detach().numpy())
